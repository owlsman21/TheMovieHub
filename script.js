...
let currentPage = 1;
let currentFilter = 'All'; // Tracks the currently selected genre (e.g., "Action", "Horror", "All")
let currentSearchTerm = ''; // Tracks the current search input value

// DOM Elements (ensure these IDs match your index.html)
...
const genreFiltersDiv = document.getElementById('genre-filters'); // New: Get the genre filter container

// This array will hold the movies after applying both genre and search filters
...
function createMovieCard(movie) {
    ...
    // Add Tailwind classes for styling the card
    ...
    // Movie Title
    const movieTitleH3 = document.createElement('h3'); // Changed to h3 for better semantic HTML
    movieTitleH3.classList.add('movie-title', 'text-xl', 'font-semibold', 'text-blue-700', 'mb-2'); // Tailwind classes
    ...
    if (movie.description) {
        ...
        movieDescriptionP.classList.add('movie-description', 'text-gray-600', 'mb-4'); // Tailwind classes
    ...
    if (movie.genres && movie.genres.length > 0) {
        ...
        genresP.classList.add('text-sm', 'text-gray-500', 'mb-3'); // Tailwind classes
        // Filter out any empty strings from the genres array before joining
    ...
    if (movie.embedId) {
        ...
        videoResponsiveDiv.classList.add('video-responsive', 'w-full'); // Tailwind class for width
...
// This function now primarily handles the rendering of the current page/single movie
function displayContent(data, isSingleMovie = false) {
    ...
    if (isSingleMovie) {
        ...
        // Hide genre filters when viewing a single movie
        if (genreFiltersDiv) {
            genreFiltersDiv.parentElement.style.display = 'none'; // Hide the parent div of genre filters
        }
        ...
        backButton.addEventListener('click', () => {
            window.location.hash = ''; // Clear hash to return to main list
        });
        movieGrid.appendChild(backButton); // Add back button above the single movie card

        movieGrid.appendChild(createMovieCard(data));

    } else {
        // Display paginated/search/genre filtered results
        pagination.style.display = 'flex'; // Show pagination
        searchResultsInfo.style.display = 'block'; // Show search info
        // Show genre filters when viewing the main list
        if (genreFiltersDiv) {
            genreFiltersDiv.parentElement.style.display = 'block'; // Show the parent div of genre filters
        }
...
function applyFiltersAndDisplay() {
    let tempMovies = [...movies]; // Start with all original movies
    ...
    // Update the global filtered list and reset to first page
    filteredAndSearchedMovies = tempMovies;
    currentPage = 1; // Always reset to the first page when filters/search change
...
function handleHashChange() {
    ...
    if (hash) {
        ...
        if (foundMovie) {
            ...
            applyFiltersAndDisplay(); // Re-apply filters and display main list
        }
    } else {
        // No hash, or hash cleared, display paginated list with current filters
...
if (searchInput) {
    searchInput.addEventListener('keyup', (event) => {
        currentSearchTerm = searchInput.value.trim(); // Update search term on each keyup
        if (event.key === 'Enter' || currentSearchTerm === '') {
            currentFilter = 'All'; // Reset genre filter on search
...
if (clearSearchButton) {
    clearSearchButton.addEventListener('click', () => {
        searchInput.value = '';
        currentSearchTerm = '';
        currentFilter = 'All'; // Reset genre filter to 'All'
        document.querySelectorAll('.genre-button').forEach(button => {
            button.classList.remove('active');
            if (button.dataset.genre === 'All') {
                button.classList.add('active');
        ...
        applyFiltersAndDisplay(); // This will display all movies with 'All' genre filter
    });
}

// Genre Filter Buttons Event Delegation
if (genreFiltersDiv) {
    genreFiltersDiv.addEventListener('click', (event) => {
        if (event.target.classList.contains('genre-button')) {
            ...
            // Update the current filter and clear search
            currentFilter = event.target.dataset.genre;
            currentSearchTerm = ''; // Clear search when genre filter is applied
            searchInput.value = ''; // Clear search input visually

            window.location.hash = ''; // Clear hash to ensure main list view
...
// --- Initial Load ---
// Display content based on hash or initial filters when the page first loads
